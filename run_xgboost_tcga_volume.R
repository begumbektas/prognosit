...
data_path <- "./data"
result_path <- "./results"
pathway <- "none" #replace with "hallmark" or with "pid" if you would like to restrict xgboost to Hallmark or to PID genes only
...
for(cohort in cohorts) {
  ...
  for(replication in 1:100) {
    if (file.exists(sprintf("%s/%s/xgboost_pathway_%s_NRMSE_replication_%d_result.RData", result_path, cohort, pathway, replication)) == FALSE) {
      ...
      error_matrix <- matrix(NA, nrow = fold_count, ncol = length(max_depth_set), dimnames = list(1:fold_count, sprintf("%g", max_depth_set)))
      ...
      for (fold in 1:fold_count) {
        ...
        for(max_depth in max_depth_set){
          ...
          model <- xgboost(data = X_train, label = y_train, objective = "reg:squarederror", eta = .2, nrounds = 1000, max_depth = max_depth, early_stopping_rounds = 15)
          y_predicted <- predict(model, X_test)
          error_matrix[fold, sprintf("%g", max_depth)] <- NRMSE((y_predicted)^3, y_test)
          }
      }
      
      max_depth_star <- max_depth_set[which.min(colMeans(error_matrix, na.rm = TRUE))]
      ...
      result <- list()
      result$NRMSE <- NRMSE((y_predicted)^3, y_test)
      result$predicted <- (y_predicted)^3
      result$actual_y <- y_test
      result$max_depth_star <- max_depth_star
      
      save("result", file = sprintf("%s/%s/xgboost_pathway_%s_NRMSE_replication_%d_result.RData", result_path, cohort, pathway, replication))
    }
  }
}
