...
source("single_kernel_regression_train.R")
source("single_kernel_regression_test.R")
library(tidyr)

cohorts <- c("TCGA-THCA") 
data_path <- "./data"
result_path <- "./results"
pathway <- "none" #replace with "hallmark" or with "pid" if you would like to restrict SVR to Hallmark genes or to PID genes only
...
for(cohort in cohorts) {
  ...
  for(replication in 1:100) {
    if (file.exists(sprintf("%s/%s/svr_pathway_%s_measure_NRMSE_replication_%d_result.RData", result_path, cohort, pathway, replication)) == FALSE) {
      ...
      for (fold in 1:fold_count) {
        ...
        for (C in C_set) {
          for (tube in tube_set){
            ...
            state <- single_kernel_regression_train(K_train, y_train, parameters)
            prediction <- single_kernel_regression_test(K_test, state)
            
            error_matrix[fold, sprintf("%g_%g", C, tube)] <- NRMSE((prediction$y)^3, y_test)
          }
        }
      }
      ...
      prediction <- single_kernel_regression_test(K_test, state)
      result <- list()
      result$predicted <- (prediction$y)^3
      result$actual_y <- y_test
      result$NRMSE <- NRMSE((prediction$y)^3, y_test)
      result$tube_multiplier <- tube_star_NRMSE_multiplier
      
      save("state", file = sprintf("%s/%s/svr_pathway_%s_measure_NRMSE_replication_%d_state.RData", result_path, cohort, pathway, replication))
      save("result", file = sprintf("%s/%s/svr_pathway_%s_measure_NRMSE_replication_%d_result.RData", result_path, cohort, pathway, replication))
    }
  }
}
